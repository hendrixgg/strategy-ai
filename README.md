...
1. python modules explicitly installed with pip install (versions and implicit dependencies found in backend/requirements.txt):
    - Flask
    - langchain
...
1. base.py: this is where the flask app is run from
    1. load_dotenv environment variables so that ChatOpenAI can work properly
    1. origins for CORS (can be limited in future, for development all origins are allowed)
    1. tasks: dict for all the tasks of the current session
    1. File system, documents loaded, vector store, llm
    ...
    1. api route to send the file structure to the UI
        1. implement path_to_dict to retrieve file structure of available documents
    1. api route to initalize a task
        1. requires: BaseTask, Task1Surfacing
    1. api route to stream the task results
        1. requires: BaseTask, Task1Surfacing
    1. api route to save the results (tasks are saved automatically after they finished running, this is just to save a copy of readable text results from hidden_files/api_output to the visible_files/ai_files so that the output can be used for future task input)
        1. requires: BaseTask, Task1Surfacing
...
1. task.py: BaseTask: abstract class that captures the essesntial functions and data that a task executed from this backend would have
    ...
    1. **NEED TO DO**: specify the format for BaseTask.generate_results() stream output with a dataclass or some reusable class
